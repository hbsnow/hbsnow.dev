...
JavaScript の sort は破壊的で、非破壊ソートがありません。そのためソートにはちょっとひと工夫が必要です。
...
`0` と `undefined` がどちらも falsy なのでややこしく、テストがないとかなり厳しい不穏なコードになります。
...
この手のコードはテストコードも fixture があって複雑だったりするので、メンテナンスのことを考えるとあまり書きたいとは思えません。

## まとめ

1. 基本的には `lodash.sortby` を使う
1. 複数の key が判定の条件にあって ASC/DESC が異なる場合には `lodash.orderby` を使う
1. ライブラリが使えない場合、`sort` を使って頑張る。必ずテストコードを書くこと
